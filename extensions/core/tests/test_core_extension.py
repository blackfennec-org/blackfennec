...
def test_create_core_extension(extension_api, type_registry, presenter_registry):
    create_extension(extension_api)
    assert type_registry.register_type_count == 6
    assert presenter_registry.register_presenter_count == 1


def test_destroy_core_extension(extension_api, type_registry, presenter_registry):
    destroy_extension(extension_api)
    assert type_registry.deregister_type_count == 6
    assert presenter_registry.deregister_presenter_count == 1


def test_everything_created_is_destroyed(extension_api, type_registry, presenter_registry):
    create_extension(extension_api)
    destroy_extension(extension_api)
